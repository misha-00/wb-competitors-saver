...
# Полный файл: авторизация одноразовыми ключами + админ-панель + скачивание фото, Excel с картинками, коллаж, ZIP.
# ВРЕМЕННО: пароль владельца захардкожен (FORCE_ADMIN_PASSWORD). Убери/замени в проде!
...
TOKEN_TTL_HOURS_DEFAULT = 24                        # срок действия ключа в часах (по умолчанию)
...
DEFAULT_SLIDES = 10
THUMB = (360, 360)
CELL_PX = (160, 160)   # ширина, высота вставляемых картинок в Excel (пиксели)
...
def admin_view():
    ...
    with st.form("issue_form"):
        ...
        with col1:
            note = st.text_input("Заметка (кому/для чего)", placeholder="Клиент Иванов / тест")
        with col2:
            ttl = st.number_input("Срок (ч)", min_value=1, max_value=168, value=TOKEN_TTL_HOURS_DEFAULT, step=1)
        count = st.number_input("Сколько ключей", min_value=1, max_value=100, value=5, step=1)
        btn = st.form_submit_button("Сгенерировать")
    if btn:
        tokens = issue_tokens(int(count), int(ttl), note.strip() if note else None)
        st.success("Ключи сгенерированы — отправь пользователям.")
        for t in tokens:
            st.code(t["token"])
        st.caption("Каждый ключ одноразовый и истекает по времени.")

    with st.expander("Список ключей (debug)"):
        store = _load_store()
        rows = []
...
def fetch_card_json(session: requests.Session, nm: str) -> dict | None:
    api = f"https://card.wb.ru/cards/v2/detail?appType=1&curr=rub&dest=-1257786&spp=0&nm={nm}"
